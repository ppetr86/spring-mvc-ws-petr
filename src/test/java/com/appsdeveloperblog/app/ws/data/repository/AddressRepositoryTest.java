...
class AddressRepositoryTest {
    ...
    @Autowired
    public AddressRepositoryTest(AddressRepository addressRepository, TestEntityManager entityManager, UserRepository userRepository) {
    ...
    @Test
    void findAllByAddressIdContainsIgnoreCase() {
        AtomicInteger counter = new AtomicInteger();
        addressRepository.findAllByAddressIdContainsIgnoreCase("A").forEach(each -> counter.incrementAndGet());

        var countWithSimpleFindALl = addressRepository.findAll().stream().filter(each -> each.getAddressId().toUpperCase().contains("A")).count();

        Assertions.assertThat(counter.get()).isEqualTo(countWithSimpleFindALl);
    }
    ...
    void findAllByUser() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        var futureListAddress = addressRepository.findAllByUser(randomUser);
